...
```json
{
  ...
  "release_upgrade": { "from": "44", "to": "45", "armed": true },
  "image_based": true
}
```
...
| `release_upgrade` | object, optional | Present **only** while dnf5 has a Fedora release upgrade stored. `from` and `to` are the two releases, both strings; `armed` says whether a restart actually installs it - which is TWO things, `status = "ready"` **and** the `/system-update` symlink, the same pair `offline_staged` is gated on, because systemd removes that symlink once `system-update.target` is reached and a `ready` transaction without it is one a restart came and went without running. It is `true` or `false` rather than absent, because "downloaded but not started" is a fact worth having and a reader with `from`/`to` and no `armed` would word its sentence wrongly. `dnf5 system-upgrade download` stops at `armed: false` and a box can sit there for days. dnf5 keeps ONE stored transaction for release upgrades and ordinary offline updates alike, so this is the fact that says the offline surface is unavailable: staging over a release upgrade cancels it, and Kempt refuses to. Published so a reader can stop OFFERING to stage rather than letting the press be how somebody finds out. Detected by comparing `system_releasever` with `target_releasever` in dnf5's transaction-state file - a COMPARISON and never a presence test, because both keys are in every `state_version = 2` file and an ordinary offline upgrade carries the same value in both. Absent, never `null`, when there is none. Additive key. |

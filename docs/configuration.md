...
| `widget_icon_size` | `auto`, `small`, `medium`, `large` | `auto` | How big the Plasma widget draws its panel icon. `auto` matches what the system tray draws at your panel's thickness (22 px on anything from a 22 px panel to a 47 px one, which covers the usual ones); the named sizes are 16, 22 and 32 px. A size the panel cannot fit falls back to `auto`, which is what makes the system tray's own slot win when the widget lives inside it. Like `refresh_interval_min`, this is stored here so the widget and the CLI share one place; the CLI itself has no icons. Validated by the **widget**, not by `kempt config set`: an unrecognized value means `auto` and is not an error. |

...
8. **Reboot detection via `dnf5 needs-restarting --json`** — returns `{reboot_required: bool, packages: [...]}` machine-readable. But cross-check against the transaction's own package list: the hardcoded "important" set is `kernel, kernel-rt, glibc, linux-firmware, systemd, dbus, dbus-broker, dbus-daemon, microcode_ctl`, and it has known false negatives on kernel detection ([dnf5#2562](https://github.com/rpm-software-management/dnf5/issues/2562), [RHBZ 2137935](https://bugzilla.redhat.com/show_bug.cgi?id=2137935), [oakleys.org.uk fix note](https://www.oakleys.org.uk/blog/2025/10/fix_dnf_needsrestarting_not_detecting_updated_kernel)). Its service-restart list is documented as "quite aggressive." *(Correction, 2026-08-26: the two tracker links behind "known false negatives on kernel detection" were fetched and do not support it. dnf5#2562 is "Inconsistencies with dnf needs-restarting", a sudo/D-Bus ENOTCONN report about `--services`; RHBZ 2137935 is "dnf needs-restarting always true", a fixed false POSITIVE in dnf4's plugin, which argues the other way. Only the oakleys.org.uk note is actually about a kernel false negative, and it is a personal blog describing AlmaLinux 9 hosts where `kernel` is not installed but `kernel-core` is. The shipped code and `docs/architecture.md` therefore make this argument from the repo's own executed behaviour instead of citing any of the three.)*

...
- [ ] **W-5 Settings round-trip, BOTH ways.** Right-click → **Configure Kempt...**.
      (a) Flip **Include Flatpak apps**, press **Apply** → the button greys out → confirm with
          `kempt config get include_flatpak`. Flip it back with Apply.
      (b) Flip it again and press **OK** (which closes the dialog) → confirm with
          `kempt config get include_flatpak` again.
      **(b) is the open question**: OK closes the dialog immediately after saving, and the writes
      are asynchronous, so if the shell tears the page down before `kempt config set` finishes the
      write could be lost. Unverifiable without a real plasmashell. If (a) works and (b) does not,
      that is the bug - say so and it gets a fix, not a doc note.

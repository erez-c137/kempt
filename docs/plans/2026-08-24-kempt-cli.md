...
### MORNING VISUAL GATE — the panel widget (Plan 2, Task W5). Also founder-gated.

Everything above verifies the engine from a terminal. These verify the FACE, and none of them can
be checked overnight: no plasmoidviewer on this box (it needs `plasma-sdk`, which needs root), and
nothing automated can judge whether a panel icon looks right. The widget is installed and listed
(`kpackagetool6 -t Plasma/Applet -l | grep kempt`) but is deliberately NOT on any panel - adding it
is W-1 and yours to do.

**Do items 1-11 above FIRST.** The widget package is installed, but the CLI it is a client of is
not: `install.sh` real mode needs a `pkexec` prompt, so overnight work stopped at the widget half.
Until item 1 has run there is no `~/.local/bin/kempt`, no root helpers and no polkit action, and
the widget will correctly show its error state with a tooltip pointing at `kempt doctor`. That is
the widget being honest, not the widget being broken - but it makes W-2 onwards meaningless.

- [ ] **W-1 Add it.** Right-click the panel → **Add Widgets...** → search **Kempt** → drag it on.
      Check the entry shows the **comb icon**, not a generic placeholder (that would mean the
      hicolor icon install did not take).
- [ ] **W-2 The badge is the truth.** Compare the badge against `kempt check | jq .actionable` -
      they must be the same number. Hover: the tooltip names the same count and, if anything is
      held, says how many. Look at the icon at your actual panel size: it should be crisp, not
      soft (the icon size is snapped to a hinted step - W5 - but only your eyes can confirm it).
- [ ] **W-3 The popup matches the JSON.** Click the icon. The System (dnf) and Apps (flatpak)
      rows and their `from → to` versions must match `kempt check | jq '.backends'`. Click a pin:
      the row moves to **Held** on the next refresh, and `kempt holds` shows it. Unpin: it comes
      back. If the offline chip appears, confirm `kempt check | jq '.risky_pending'` is non-empty.
- [ ] **W-4 Update Now, end to end.** Press it. The real auth dialog appears; the run happens on
      your configured surface; when it finishes the spinner stops and a one-line summary appears.
      **Watch the timing**: the spinner must stay up for the WHOLE transaction. Stopping early was
      a real bug (the widget read dnf's own writes to `/var/lib/rpm` as the run ending) - fixed and
      probe-covered, but this is the only place it can be confirmed against a real dnf.
- [ ] **W-5 Settings round-trip: THREE ways to leave the dialog.** Right-click → **Configure
      Kempt...**. Every setting is written by an asynchronous `kempt config set`, so the question
      is not whether Apply works but whether the WRITE survives however the dialog is left. Run all
      three, in order, and check each one with `kempt config get include_flatpak` - the value it
      prints is the only evidence; the button greying out is not.
      (b1) **Apply, wait, close.** Flip **Include Flatpak apps**, press **Apply**, count five
           seconds, then close the dialog. `kempt config get include_flatpak` → the new value.
      (b2) **Apply, then immediate Escape.** Flip it back, press **Apply** and hit **Escape** at
           once, without waiting. Expect either a prompt (the page still has a write in flight, so
           it is still "unsaved" and the shell should ask) or a clean close - then
           `kempt config get include_flatpak` → the value you just applied. A dialog that closes
           silently AND leaves the old value is the failure.
      (b3) **OK.** Flip it again and press **OK**, which saves and closes in one move.
           `kempt config get include_flatpak` → the new value.
      **None of the three is presumed to work.** All are unverifiable without a real plasmashell:
      the shell may tear the page down while `kempt config set` is still running, and that hazard
      is identical whether the dialog was closed, escaped or OK'd. Report each one's result
      separately, pass or fail - "b1 and b3 fine, b2 lost it" is a usable bug report and "settings
      work" is not. Any failure here gets a fix, not a doc note.
      (c) While in there: change something and press **Escape**. It must ASK rather than discarding
          silently. Then untick **"Apply updates without asking for confirmation"** and confirm the
          surface radios grey out but KEEP your selection; tick it back and the selection returns.
      (d) Remember settings take **up to 30 seconds** to reach the panel (the config-file watch).
          Waiting half a minute before calling something broken is part of the test.
- [ ] **W-6 Screenshot.** Spectacle the panel icon with a badge plus the open popup → save into
      `docs/` and swap it for the placeholder comment in README.md.
- [ ] **W-7 Only if you staged an offline update in item 9 above:** after the reboot, the badge
      must reflect the harvest without you touching anything.

If any of W-1..W-5 fails, stop and report it rather than working around it - each one is a
contract something automated already claims to hold.

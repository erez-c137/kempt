...
2. `upkeep check` → real pending counts, no auth dialog (refresh action is allow_active=yes). To compare against dnf reality use `jq '.backends.dnf.items|length' ~/.local/state/upkeep/state.json` vs `dnf5 check-update` — NOT `.actionable`, which is dnf+flatpak MINUS holds with multilib twins collapsed and obsolete rows filtered, so it legitimately differs from a raw line count.
3. `upkeep hold dnf:<some pending pkg>` → `upkeep check` → actionable drops by one.
4. `upkeep update` in a terminal → ONE auth dialog → real run → summary shows old→new versions; held package in "Held (skipped)". NOTE: if session-critical packages are pending, the [u]/[s]/[a] recommendation prompt appears FIRST — expected, not a bug; answer u for this step. (One auth DIALOG but two `upkeep-apply` invocations — dnf then flatpak; the second rides polkit's auth_admin_keep cache.)
...
9. Optional (fits this box's pending reboot): `upkeep update --surface=offline` → staged notification. **Negative step first (do this before rebooting):** run `upkeep check` — ideally after some unrelated rpm change — and confirm `~/.local/state/upkeep/offline_staged.json` still exists, NO new history entry appeared and NO notification fired. A pending stage is gated on the boot session, so nothing short of a reboot can consume it. Then, after the next natural reboot, `upkeep check` → harvest history entry + notification.
...
11. Wrong-outcome check: `./install.sh --uninstall` and DISMISS the auth dialog → exit 1 with a message naming the half-removed state (the `~/.local/bin/upkeep` symlink is gone, the root helpers and polkit action are still installed) and telling you to re-run. Then either finish the uninstall (`./install.sh --uninstall` again, accept) and reinstall with `./install.sh`, or just re-run `./install.sh` to put the symlink back. Either way, end with a working install: `upkeep check`.
...
- Item 9: a pending stage now survives ANY pre-reboot rpm change, not just a live `upkeep update` — the harvest is gated on the boot session (`boot_id` in the marker), so a manual `dnf install`, a transient snapshot failure or a live Upkeep run can no longer consume the marker or be reported as the staged result. The baseline rebase still runs on top of that, so the post-reboot diff of a live-run-then-reboot sequence contains only the staged transaction.

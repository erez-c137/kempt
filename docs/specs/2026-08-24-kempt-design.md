...
- **Popup:** header "N updates available", scrollable pending list grouped System (dnf) / Apps (flatpak) with `name old → new`, buttons **Update Now** and **Refresh**, gear icon → standard plasmoid config dialog. Each row has a pin toggle (hold/unhold); held items move to a separate "Held" section showing the waiting version.
  - **DELTA (implemented, Plan 3): the primary action moved to a FOOTER, and the header is one row.** `PlasmaExtras.Representation` gives a page `header`, content and `footer`; the heading is the one row Plasma's contract lets the containment REPLACE (`ContainmentDrawsPlasmoidHeading`, and every shipped applet treats heading contents as expendable), so Update Now cannot live there - it must exist on every host, in every containment. A footer also keeps it in reach while a 1200-row list scrolls. `org.kde.plasma.vault` is the precedent. The header keeps the pending count, a `view-refresh` icon button (`Check for Updates`, also registered as a contextual action so the tray's More-actions menu and the icon's right-click menu carry it) and the gear, which hides itself in the system tray because the tray draws a heading with a gear of its own. Everything that was a MESSAGE in that toolbar - the stale explanation, the risky warning, the last action's report - is a `Kirigami.InlineMessage` in the content area now.
  - **DELTA (implemented, Plan 3): Update Now is HIDDEN, not disabled, when there is nothing to run.** An up-to-date box has no run to start, so there is no action to offer rather than an action being refused. The up-to-date state is the header's `Up to date` plus a `PlaceholderMessage` reading `Everything is up to date` - suppressed when the only pending updates are held, since the Held group is then on screen carrying the truth.
  - **DELTA (implemented, Plan 3): the restart is ASKED FOR and never performed.** The popup's restart message calls `org.kde.LogoutPrompt.promptReboot` over the session bus, which opens KDE's own cancellable confirmation screen; the second family of methods on `plasma-shutdown` takes no answer and simply reboots, and Kempt must never call it. Shown in every state including up to date (you can owe a restart with nothing pending), closable for the session and not to disk, and gated on the new `restart_reminder` config key (default true). With the reminder off the status line still ends `restart pending`: the fact is not the reminder.
  - **DELTA (implemented, Plan 3): the state file carries a LIVE `reboot_needed`.** `kempt check` writes whether a restart is owed right now, asked fresh from local facts (cache-only, repo-less `needs-restarting`). Additive schema-1 key; `false` means "nothing to say", never "no restart needed", because the underlying command can exit non-zero having computed no verdict at all. Distinct from a history entry's `reboot_needed`, which is a fact about that run and would go on claiming a restart after the user had performed one.
  - **DELTA (implemented, Plan 3): the popup's last-run data comes from `kempt summary --json`.** The persistent `Last update <relative> · N packages` row and the transient post-run line both read the newest history entry as DATA, through `Logic.lastRunOf`. Never the human `kempt summary`, whose first line is an ISO timestamp - which is what the popup used to paste into its message area after a run. Re-deriving the counts from the rendered text would put a second, lossier copy of `render_summary`'s rules in the widget, and the two would drift.
...
**Risky-transaction detection:** if the pending transaction touches session-critical packages (kernel, systemd, glibc, dbus, mesa, qt6*/kf6*, plasma-workspace), Kempt recommends the offline path before proceeding. CLI half (Plan 1, Task 13.5): `kempt check` publishes `risky_pending` in the state JSON (additive schema-1 key; consumers tolerate absence); interactive terminal updates prompt [u]pdate live / [s]tage offline / [a]bort; detached surfaces get a heads-up notification and proceed. Widget half (Plan 2): one click on that recommendation, named **Install on Next Restart** since Plan 3. Live surfaces stay available; the user always decides. Default surface remains `terminal` (live) - Erez's call to flip the default to `offline`.
